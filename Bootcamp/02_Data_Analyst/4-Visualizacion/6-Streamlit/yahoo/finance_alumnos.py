...
# Para ejecutar el programa que estamos haciendo: python -m streamlit run finance_alumnos.py
# Para cerrar la conexion en la terminal: control C

st.set_page_config(page_title='My Finance App', layout='wide') #Caracteristicas de la pagina


st.write('# Precio Stock Online') #Titulo central (acepta markdown)
...
inicio=st.sidebar.date_input('Selecciona la fecha de inicio',
                            datetime.date(2022,1,5))
fin=st.sidebar.date_input('Selecciona la fecha de fin',
...
c2.metric(label= 'Último valor de cierre',
...
c3.metric(label= 'Último valor de apertura',
...
fig_row1 = make_subplots(rows=1, cols=2,
                        subplot_titles = (f'Valor de apertura de {producto}',
                                        f'Valor de cierre de {producto}'))
fig_row1.add_trace(go.Scatter(x = productoDf.index,
                              y = productoDf.Open),
                              row=1,
                              col=1)
fig_row1.add_trace(go.Scatter(x = productoDf.index,
...
fig_row2 = make_subplots(rows=1, cols=2,
                        subplot_titles = (f'Valor maximo de {producto}',
                                        f'Valor minimo de {producto}'))
fig_row2.add_trace(go.Scatter(x = productoDf.index,
                              y = productoDf.High),
                              row=1,
                              col=1)
fig_row2.add_trace(go.Scatter(x = productoDf.index,
